...
if [[ ($RESTO_DEBUG = 1 || $RESTO_DEBUG = '1' || $RESTO_DEBUG = 'true') && ($PHP_ENABLE_XDEBUG = 1 || $PHP_ENABLE_XDEBUG = '1' || $PHP_ENABLE_XDEBUG = 'true') ]]; then
  ...
  sed -i 's/"localhost"/192.168.0.138/' /etc/php/${PHP_VERSION}/mods-available/xdebug.ini
  ...
  echo "xdebug.client_host = 192.168.0.138" >>/etc/php/${PHP_VERSION}/mods-available/xdebug.ini
